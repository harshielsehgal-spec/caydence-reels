...
const ReelCard = ({ reel, athleteId, onAnalyze, onOpenTips, onOpenLeaderboard, isVisible, userScore = 20, isJoined = false, attemptHistory = [] }: ReelCardProps) => {
  ...
  const handleSave = (e: React.MouseEvent) => {
    ...
    setSaved(newSaved);
    if (newSaved) toast({ title: "Saved to your drills" });
  };
  ...
  const handleShare = async (e: React.MouseEvent) => {
    ...
    const shareData = { title: reel.title, text: "Check out this Caydence drill!", url: shareUrl };
    if (navigator.share) {
  ...
  return (
    <div className="h-screen w-full flex-shrink-0 snap-start snap-always flex items-center justify-center bg-background">
      <div className="relative w-full max-w-[480px] h-full mx-auto lg:max-h-[calc(100vh-2rem)] lg:my-4 lg:rounded-2xl lg:overflow-hidden">
        ...
        <button onClick={togglePlay} className="absolute inset-0 z-10">
          <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-200 ${isPlaying ? 'opacity-0 hover:opacity-100' : 'opacity-100'}`}>
            <div className="w-16 h-16 rounded-full bg-background/40 backdrop-blur-sm flex items-center justify-center">
              {isPlaying ? <Pause className="w-7 h-7 text-foreground" /> : <Play className="w-7 h-7 text-foreground fill-foreground ml-1" />}
            </div>
          </div>
        </button>
        ...
        {/* Challenge Banner - below top bar */}
        <div className="absolute top-16 left-4 right-4 z-20">
          <ChallengeBanner creatorName={reel.creator_username} />
        </div>
        ...
        <div className="absolute inset-x-0 bottom-0 z-20">
          ...
          <div className="relative px-4 pb-24 lg:pb-8 pt-16">
            ...
            {/* Progress Strip */}
            <ProgressStrip attempts={attemptHistory} />
